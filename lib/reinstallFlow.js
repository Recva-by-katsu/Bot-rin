...
async function testIsoLinkFromVps(sshSession, isoUrl) {
  // Tes link dari VPS target: curl -sL -r 0-0 -w '%{http_code}'
  const cmd = `curl -sL -r 0-0 -o /dev/null -w '%{http_code}' ${shellEscape(isoUrl)} 2>/dev/null || wget --spider -S ${shellEscape(isoUrl)} 2>&1 | grep -o "[0-9]\\{3\\}" | tail -n 1`;
  const res = await sshSession.exec(cmd);
  const code = res.stdout.trim().slice(-3);
  if (code === '200' || code === '206') return true;
  // Coba lagi dengan curl tanpa -r
  const cmd2 = `curl -sL -o /dev/null -w '%{http_code}' ${shellEscape(isoUrl)}`;
  const res2 = await sshSession.exec(cmd2);
