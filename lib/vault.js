...
class Vault {
  ...
  addOrUpdateAccount(userId, tokenPlain, username, label, meta = {}) {
    const uid = String(userId);
    const all = this._readAll();
    if (!all[uid]) all[uid] = [];
    // Merge meta (mis. { trial_mode }) tanpa menimpa field yang sudah ada dengan undefined
    const cleanMeta = {};
    for (const [k, v] of Object.entries(meta)) if (v !== undefined) cleanMeta[k] = v;
    ...
    if (existing) {
      ...
      Object.assign(existing, cleanMeta);
  ...
  // Update field meta akun (mis. { trial_mode }) tanpa menyentuh token
  updateAccountMeta(userId, accountId, patch) {
    const all = this._readAll();
    const uid = String(userId);
    const acc = (all[uid] || []).find(a => a.id === accountId);
    if (!acc) return null;
    for (const [k, v] of Object.entries(patch)) if (v !== undefined) acc[k] = v;
